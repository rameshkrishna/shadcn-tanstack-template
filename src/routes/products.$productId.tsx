...
function parseCommaList(text: string): Array<string> {
  return text
    .split(",")
...
export const Route = createFileRoute("/products/$productId")({
  validateSearch: (search: Record<string, unknown>): ProductDetailSearch => ({
    tab:
      search.tab === "etsy" || search.tab === "details"
        ? search.tab
        : undefined,
  }),
...
function buildEtsyForm(
  ...
  const fields: EtsyListingFields = listing?.fields ?? {
    ...defaultFields,
    tags: [],
    materials: [],
  }
...
function ProductDetail() {
  ...
  async function handleResync() {
    ...
    setResyncing(true)
    try {
      const fresh = await resyncProduct(product.id)
      setForm({
        name: fresh.name,
        sku: fresh.sku,
      ...
      toast.success("Product resynced from source")
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to resync product"
      )
    } finally {
      setResyncing(false)
    }
  ...
  async function handleSaveEtsyListing() {
    ...
    const tags = parseCommaList(etsyForm.tagsText).slice(0, ETSY_MAX_TAGS)
    const materials = parseCommaList(etsyForm.materialsText).slice(
      0,
      ETSY_MAX_MATERIALS
    )
    ...
    await upsertChannelListing(product.id, "etsy", {
      ...
      overrides: {
        ...
        description:
          etsyForm.description !== product.description
            ? etsyForm.description
            : undefined,
        price: etsyForm.price !== product.price ? etsyForm.price : undefined,
        stock:
          etsyForm.quantity !== product.stock ? etsyForm.quantity : undefined,
        sku:
          etsyForm.vendorSku !== product.sku ? etsyForm.vendorSku : undefined,
      },
  ...
  async function handleGenerateEtsyCopy() {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to generate Etsy copy"
      )
    } finally {
  ...
  const tagList = parseCommaList(etsyForm.tagsText)
  const overLengthTags = tagList.filter(
    (tag) => tag.length > ETSY_TAG_MAX_LENGTH
  )
  const materialList = parseCommaList(etsyForm.materialsText)
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            className="gap-2"
            disabled={resyncing || !product.sourceUrl}
            onClick={handleResync}
            title={
              product.sourceUrl
                ? undefined
                : "This product has no source URL to resync from"
            }
          >
            {resyncing ? (
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This removes "{product.name}" from your inventory. This can't
                  be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <Button
            onClick={activeTab === "etsy" ? handleSaveEtsyListing : handleSave}
          >
            {activeTab === "etsy"
              ? etsyListing
                ? "Save Etsy Listing"
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setForm({ ...form, imagePrompt: e.target.value })
                    }
                  />
          ...
          <motion.div initial="hidden" animate="show" variants={fadeInUp}>
            <Card className="border-border/50 bg-card/60 shadow-sm backdrop-blur-md">
              <CardHeader className="flex flex-row items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  {etsyListing
                    ? `Last saved ${new Date(etsyListing.updatedAt).toLocaleString()}`
                    : "Not listed on Etsy yet"}
                </p>
                <div className="flex items-center gap-2">
                  {etsyListing && (
                    <Badge variant="secondary">
                      {etsyListing.status === "ready"
                        ? "Ready to export"
                        : etsyListing.status}
                    </Badge>
                  )}
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEtsyForm({ ...etsyForm, vendorSku: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEtsyForm({
                          ...
                          fields: {
                            ...etsyForm.fields,
                            taxonomyCategory: e.target.value,
                          },
                        })
                      }
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEtsyForm({ ...etsyForm, title: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setEtsyForm({ ...etsyForm, description: e.target.value })
                    }
                  />
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value: EtsyListingFields["whoMade"]) =>
                        setEtsyForm({
                          ...etsyForm,
                          fields: { ...etsyForm.fields, whoMade: value },
                        })
                      }
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value: EtsyListingFields["whenMade"]) =>
                        setEtsyForm({
                          ...etsyForm,
                          fields: { ...etsyForm.fields, whenMade: value },
                        })
                      }
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(
                        value: EtsyListingFields["renewalOption"]
                      ) =>
                        setEtsyForm({
                          ...etsyForm,
                          fields: { ...etsyForm.fields, renewalOption: value },
                        })
                      }
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setEtsyForm({
                          ...etsyForm,
                          fields: { ...etsyForm.fields, primaryColor: value },
                        })
                      }
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setEtsyForm({
                          ...etsyForm,
                          fields: { ...etsyForm.fields, secondaryColor: value },
                        })
                      }
                    >
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEtsyForm({ ...etsyForm, tagsText: e.target.value })
                    }
                    placeholder="e.g. gold necklace, minimalist jewelry"
                  />
                  {tagList.length > ETSY_MAX_TAGS && (
                    <p className="text-xs text-destructive">
                      Only the first {ETSY_MAX_TAGS} tags will be saved — the
                      rest are dropped.
                    </p>
                  )}
                  {overLengthTags.length > 0 && (
                    <p className="text-xs text-destructive">
                      Over Etsy's {ETSY_TAG_MAX_LENGTH}-character tag limit:{" "}
                      {overLengthTags.join(", ")}
                    </p>
                  )}
                </div>
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="etsyMaterials">
                      Materials (comma-separated)
                    </Label>
                    <span
                      className={cn(
                        "text-xs",
                        materialList.length > ETSY_MAX_MATERIALS
                          ? "text-destructive"
                          : "text-muted-foreground"
                      )}
                    >
                  ...
                  {materialList.length > ETSY_MAX_MATERIALS && (
                    <p className="text-xs text-destructive">
                      Only the first {ETSY_MAX_MATERIALS} materials will be
                      saved — the rest are dropped.
                    </p>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between rounded-lg border border-border p-3">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setEtsyForm({
                        ...etsyForm,
                        fields: { ...etsyForm.fields, isSupply: checked },
                      })
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between rounded-lg border border-border p-3">
                  <div className="space-y-0.5">
                    <Label htmlFor="etsyPersonalization">
                ...
                {etsyForm.fields.personalization && (
                  <div className="space-y-1.5">
                    <Label htmlFor="etsyPersonalizationInstructions">
